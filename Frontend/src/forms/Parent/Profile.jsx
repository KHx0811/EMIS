  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3000/api/parents/profile', {
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={4} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <Typography variant="h5" sx={{ color: '#e0e0e0', mb: 1, textAlign: 'center' }}>
              {profileData?.name || 'Parent Name Not Available'}
            </Typography>
          ...
          <Grid item xs={12} md={8}>
            <Box sx={{ mb: 3 }}>
              ...
              <ProfileItem 
                ...
                label="Parent ID"
                value={profileData?.parent_id || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<Badge sx={{ color: '#00deb6' }} />}
                label="Student ID"
                value={profileData?.student_id || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<Person sx={{ color: '#00deb6' }} />}
                label="Gender"
                value={profileData?.gender || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<Person sx={{ color: '#00deb6' }} />}
                label="Relation"
                value={profileData?.relation || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<CalendarMonth sx={{ color: '#00deb6' }} />}
                label="Date of Birth"
                value={formatDate(profileData?.date_of_birth) || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<Person sx={{ color: '#00deb6' }} />}
                label="Qualification"
                value={profileData?.qualification || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<Phone sx={{ color: '#00deb6' }} />}
                label="Phone Number"

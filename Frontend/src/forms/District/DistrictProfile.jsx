...
import { School, CalendarMonth, Person, Email, Badge, LocationCity, Public } from '@mui/icons-material';
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3000/api/districts/profile', {
      ...
      >
        <Grid container spacing={3}>
          <Grid item xs={12} md={4} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            >
              {profileData?.districthead_name?.charAt(0) || 'D'}
            </Avatar>
            <Typography variant="h5" sx={{ color: '#e0e0e0', mb: 1, textAlign: 'center' }}>
              {profileData?.districthead_name || 'District Name Not Available'}
            </Typography>
          ...
          <Grid item xs={12} md={8}>
            <Box sx={{ mb: 3, p: 2}}>
              ...
              <ProfileItem 
                icon={<LocationCity sx={{ color: '#00deb6' }} />}
                label="District Name"
                value={profileData?.district_name || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<School sx={{ color: '#00deb6' }} />}
                label="Number of Schools"
                value={profileData?.number_of_schools || 'Not Available'}
              />
              
              <ProfileItem 
                icon={<Public sx={{ color: '#00deb6' }} />}
                label="State"
